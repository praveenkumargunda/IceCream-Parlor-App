...
def add_allergen(conn, ingredient_name):
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT id, allergen FROM ingredients WHERE name = ?", (ingredient_name,))
        ...
        if ingredient:
            if not ingredient[1]:  
                ...
                print(f"{ingredient_name} is already marked as an allergen.")
        else:
            cursor.execute("INSERT INTO ingredients (name, allergen) VALUES (?, 1)", (ingredient_name,))
            print(f"{ingredient_name} added as a new allergen.")
            conn.commit()
    except Exception as e:
    ...
    try:
        ...
        if not suggestions:
            ...
            for suggestion in suggestions:
                print(f"{suggestion[0]}. {suggestion[1]} suggests {suggestion[2]} (Allergens: {suggestion[3]})")
    except Exception as e:
    ...
    while True:
        try:
            ...
            if choice == '1':
                ...
                view_customer_suggestions(conn)
            elif choice == '9':
